...
namespace UI_InputSystem.Base
{
    ...
    {
        public static Dictionary<TInputAction, TInputType> GetAvailableInputs<
            TInputAction, TInputType, TInputValue>()
            where TInputType : UIInput<TInputAction, TInputValue>
        {
            ...
            {
                Debug.Log(
                    $"Checking gameObject {inputType.gameObject.name}.");
                if (newInputs.ContainsKey(inputType.InputID))
                {
                    Debug.LogError(
                        $"Duplicates of {inputType.InputID} InputAction on gameObject {inputType.gameObject.name}! \n Please use only 1 input for each action!");
                    continue;
                }
